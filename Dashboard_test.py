...
def load_contract1():
    ...
    with open(Path('./Ventidex_abi.json')) as f:
...
################################################################################
st.markdown("## Choose your index")
artwork_name = st.text_input("Enter the name of your portfolio")
artist_name = st.text_input("Enter the token name")
...
tokens = contract.functions.tokenURI().call()
...
if st.button("Appraise Artwork"):
    ...
    tx_hash = contract.functions.newDistribution(
        token_id,
...
if st.button("Get Appraisal Reports"):
    appraisal_filter = contract.events.Portfolio.createFilter(
        fromBlock=0,
